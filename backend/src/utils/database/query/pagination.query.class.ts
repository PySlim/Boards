...
class PaginationQuery {
    ...
    private Pagination(
        ...
        const initialQuery= `${this.chain} ORDER BY ${this.sortField} ${this.desc ? 'DESC' : 'ASC'} LIMIT ${this.limit} OFFSET ${offset}`;
        this.chain= this.removeDuplicateOrderBy(initialQuery)
        return this;
    }
    ...
    private removeDuplicateOrderBy(sqlQuery: string) {
        const lowerCaseQuery = sqlQuery.toLowerCase();
        const firstOrderByIndex = lowerCaseQuery.indexOf('order by');
        if (firstOrderByIndex === -1) return sqlQuery;
        const secondOrderByIndex = lowerCaseQuery.indexOf('order by', firstOrderByIndex + 'order by'.length);
        if (secondOrderByIndex !== -1) {
            return sqlQuery.substring(0, secondOrderByIndex);
        }
        return sqlQuery;
    }
}
