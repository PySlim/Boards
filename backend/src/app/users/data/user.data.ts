...
class UserData implements  UserDataInterface{
    ...
    async GetUserByEmail(email: string, next: NextFunction): Promise<any> {
        try{
            const result = await this.dataSet.selectWh.exec('*',{email:email})
            return result[0]
        }catch(error){
            if (error instanceof ExpressReviewsError) next(error)
            next(new ExpressReviewsError("Failed to get user by email.",
                ConstantsResponse.INTERNAL_SERVER_ERROR, "DataError", "UserData.GetUserByEmail", error));
        }
    }
