...
class UsersService implements UsersServiceInterface{
    ...
    async Update(req: Request, next: NextFunction): Promise<any> {
        try {
            ...
            const userSearchByUsername: userInterfaceData = await UserData.GetUserByUserName(body.username,next)
            if(userSearchByUsername){
                if(userSearchByUsername.id.toString() !== id){
                    ThrowErrorHandler(new ExpressReviewsError('Username is already in use',
                        ConstantsResponse.FORBIDDEN, 'Validation Error', 'UpdateServiceUsers'), next)
                    return
                }
            }
            if(body.email){
                const userSearchByEmail: userInterfaceData = await UserData.GetUserByEmail(body.email, next)
                if(userSearchByEmail){
                    if(userSearchByEmail.id.toString() !== id){
                        ThrowErrorHandler(new ExpressReviewsError('Email is already in use',
                            ConstantsResponse.FORBIDDEN, 'Validation Error', 'UpdateServiceUsers'), next)
                        return
                    }
                }
